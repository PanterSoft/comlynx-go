...
// New opens a connection and returns a ready Client. log may be nil for
// silent operation. The caller may override the destination address via
// SetDestination before the first read.
...
func (c *Client) Ping(multi bool) ([]Address, error) {
	...
	for time.Until(deadline) > 0 {
		frame, err := readFrame(c.t, time.Until(deadline))
		if err != nil {
		...
		if err != nil {
			c.log("discard malformed frame: %v", err)
			continue
		}
		msg, err := decodeMessage(body)
		if err != nil {
			c.log("discard undecodable reply: %v", err)
			continue
		}
